...
void register_ethernet(void)
{
	...
	eth_spinet_config_t spinet_config;
		
    eth_mac_config_t mac_config = ETH_MAC_DEFAULT_CONFIG();
    mac_config.smi_mdc_gpio_num = -1;
    mac_config.smi_mdio_gpio_num = -1;	
    
	esp_eth_mac_t *mac = esp_eth_mac_new_spinet(&spinet_config, &mac_config);	
	
    eth_phy_config_t phy_config = ETH_PHY_DEFAULT_CONFIG();
    phy_config.autonego_timeout_ms = 0;
    phy_config.reset_gpio_num = -1;
    esp_eth_phy_t *phy = esp_eth_phy_new_spinet(&phy_config);
	
    esp_eth_config_t eth_config = ETH_DEFAULT_CONFIG(mac, phy);
    ESP_ERROR_CHECK(esp_eth_driver_install(&eth_config, &eth_handle));	
	
    mac->set_addr(mac, (uint8_t[]) {
        0xA6, 0x99, 0x91, 0xAD, 0x9D, 0x6F
    });
	...
	ESP_ERROR_CHECK(esp_netif_up(eth_netif));
